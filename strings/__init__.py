...
def get_command(command, lang=None):
    ...
    if not data:
        return []
    if lang:
        return list({cmd.lower() for cmd in data.get(lang, data.get("pt", []))})
    all_commands = set()
...
def get_string(lang: str):
    return languages.get(lang, languages["pt"])
...
def replace_placeholders(
        text: str, lang_data: dict, outer_key: str = "", lang_code: str = "pt"
) -> str:
...
def update_helpers(data: dict, lang_code: str = "pt"):
...
if "pt" not in languages:
    languages["pt"] = load_yaml(os.path.join("strings", "langs", "pt.yml"))
    languages_present["pt"] = languages["pt"]["name"]

languages["pt"] = update_helpers(languages["pt"], "pt")
...
for filename in os.listdir(os.path.join("strings", "langs")):
    if filename.endswith(".yml") and filename != "pt.yml":
        ...
        for key in languages["pt"]:
            if key not in languages[lang_name]:
                languages[lang_name][key] = languages["pt"][key]
        try:
